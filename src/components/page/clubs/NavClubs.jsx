...
function NavClubs() {
	return (
		<div className="sticky top-0 z-50 bg-[#fbf9f5]/70 backdrop-blur-md border-b border-[#2f4e4c]/10 transition-all duration-500 w-full h-18 shadow-sm flex items-center justify-between px-4">
			...
			<NavLink to="/" className="text-4xl font-bold eb-garamond-400 ml-10 text-[#456462] cursor-pointer hover:scale-[1.02] hover:opacity-80 transition-all duration-300">
				ELibra.
			</NavLink>

			<div className="flex pl-21 items-center space-x-4">
				<NavLink to="/bookshelf" className={({isActive}) => `relative px-3 py-1 eb-garamond-500 text-[22px] ${isActive? 'text-[#456462]': 'text-zinc-500'} transition-colors duration-300 group`}>
					Bookshelf
				</NavLink>
				...
				<NavLink to="/clubs" className={({isActive}) => `relative px-3 py-1 eb-garamond-600 text-[22px] ${isActive? 'text-[#456462]': 'text-zinc-500'} transition-transform duration-300 ease-out hover:scale-105 active:scale-95 group`}>
					Clubs
